...
# ============================================================
# ARUCO DETECTOR  (Tuned parameters + temporal buffer)
# ============================================================
class ArUcoDetector:
    def __init__(self, dict_type=ARUCO_DICT_TYPE, lose_frames=8):
        ...
        # Temporal buffer: marker tetap "hidup" selama lose_frames setelah hilang
        self.lose_frames = lose_frames
        self._tracker = {}  # {marker_id: {"lost_count": int, "data": dict}}

    def detect(self, frame):
        """Detect ArUco markers with temporal stability."""
        ...
        # Update tracker
        for mid in current:
            self._tracker[mid] = {"lost_count": 0, "data": current[mid]}

        remove_ids = []
        for mid in self._tracker:
            if mid not in current:
                self._tracker[mid]["lost_count"] += 1
                if self._tracker[mid]["lost_count"] > self.lose_frames:
                    remove_ids.append(mid)
        for mid in remove_ids:
            del self._tracker[mid]

        return [t["data"] for t in self._tracker.values()]
