...
# --- IP Camera Lite (HP sebagai kamera via MJPEG HTTP stream) ---
# CAMERA_INDEX  = "http://10.166.185.6:8081/video"
# IP_CAM_USERNAME = "admin"   # <-- Ganti dengan username IP Camera Lite
# IP_CAM_PASSWORD = "admin"   # <-- Ganti dengan password IP Camera Lite
...
# Path ke model pose_landmarker (lite = cepat, full = akurat)
MP_POSE_MODEL = "pose_landmarker_lite.task"

# --- YOLO Object Detection (bearing, kotak susu, dll) ---
CUSTOM_MODEL  = "Training bearing dummy V6.pt"
COCO_MODEL    = "yolo11n.pt"
DETECT_MODE_MODEL = "custom"
YOLO_CONF     = 0.40
YOLO_CLASSES  = [0]    # hanya deteksi bearing (class 0 dari custom model)
...
ARUCO_VALID_IDS = {0, 1, 2, 3, 4}   # hanya terima ID 0-4, sisanya diabaikan
...
# MediaPipe Tasks API (baru — menggantikan mp.solutions yang sudah dihapus)
from mediapipe.tasks import python as mp_python
from mediapipe.tasks.python import vision as mp_vision
from mediapipe.tasks.python.vision import drawing_utils as mp_drawing
from mediapipe.tasks.python.vision import drawing_utils

# Connections untuk draw_landmarks
...
# ============================================================
# IP CAMERA LITE STREAM (MJPEG via HTTP requests)
# ============================================================
class IPCameraStream:
    """
    Stream dari IP Camera Lite (HP) via MJPEG over HTTP.
    Menggunakan requests untuk handle auth (Basic/Digest/none).
    Parsing manual JPEG frames dari raw byte stream.
    Interface sama dengan CameraStream: .read(), .stop(), .start()
    """
    ...
    def __init__(self, url, username="admin", password="admin",
        ...
        # Coba koneksi awal untuk validasi
        self._connect()

    def _connect(self):
        """Establish HTTP connection with auth negotiation."""
        print(f"[IPCam] Menghubungkan ke {self.url} ...")
        ...
        try:
            ...
            # Jika Basic gagal, coba Digest Auth
            if resp.status_code == 401:
                print("[IPCam] Basic Auth gagal, mencoba Digest Auth...")
                resp = requests.get(self.url, stream=True, timeout=10,
                                    auth=HTTPDigestAuth(self.username, self.password))

            # Jika masih gagal, coba tanpa auth
            if resp.status_code == 401:
                print("[IPCam] Digest Auth gagal, mencoba tanpa auth...")
                headers = {
            ...
            if resp.status_code != 200:
                print(f"[IPCam] ERROR: Server mengembalikan status {resp.status_code}")
                return

            self._response = resp
            content_type = resp.headers.get('Content-Type', 'tidak diketahui')
            print(f"[IPCam] Terhubung! Content-Type: {content_type}")
            print(f"[INFO] IP Camera Lite: {self.actual_w}x{self.actual_h} (estimasi)")

        except requests.exceptions.ConnectionError:
            print("[IPCam] GAGAL: Tidak bisa terhubung ke server.")
            print("  -> Pastikan IP Camera Lite sudah di-start dan HP + PC di WiFi yang sama.")
        except requests.exceptions.Timeout:
            print("[IPCam] GAGAL: Koneksi timeout setelah 10 detik.")
            print("  -> Pastikan alamat IP dan port sudah benar.")
        except Exception as e:
            print(f"[IPCam] GAGAL: {type(e).__name__}: {e}")

    def start(self):
        if self._response is None:
            print("[IPCam] Tidak bisa start — koneksi gagal.")
            self.stopped = True
            return self
        threading.Thread(target=self._loop, daemon=True).start()
        return self
    ...
    def _loop(self):
        """Background thread: baca MJPEG stream, parse JPEG frames."""
        buf = b""
        try:
            for chunk in self._response.iter_content(chunk_size=4096):
                ...
                # Parse semua frame JPEG yang lengkap dari buffer
                while True:
            ...
            print(f"\n[IPCam] Stream error: {type(e).__name__}: {e}")
        finally:
            self.stopped = True
            if self._response:
...
# ============================================================
# MEDIAPIPE POSE DETECTOR (PoseLandmarker — ringan & cepat)
# (Pengganti YOLOPoseDetector dari V2.6b)
# ============================================================
class MediaPipePoseDetector:
    """
    MediaPipe PoseLandmarker (Tasks API) — hanya pose (33 titik).
    Jauh lebih ringan dari HolisticLandmarker karena TIDAK memproses
    face landmarks dan hand landmarks.

    Pre-processing (resize + cvtColor) menggunakan UMat OpenCL (iGPU).
    Inference dilakukan di CPU.

    Output 'persons' berisi:
      - bbox: (x1, y1, x2, y2) dari pose_landmarks
      - pose_landmarks: list[NormalizedLandmark] (33 titik)
      - right_hand_landmarks: None (tidak tersedia di PoseLandmarker)
      - left_hand_landmarks: None (tidak tersedia di PoseLandmarker)
    """
    ...
    def __init__(self,
        ...
        if model_path is None:
            ...
            model_path = os.path.join(script_dir, MP_POSE_MODEL)
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"Model tidak ditemukan: {model_path}\n"
                f"Download dari: https://storage.googleapis.com/mediapipe-models/"
                f"pose_landmarker/pose_landmarker_lite/float16/latest/pose_landmarker_lite.task"
            )
        ...
        print(f"[INFO] MediaPipe PoseLandmarker (lite) init (process_width={process_width})")

    def _preprocess(self, frame):
        """
        Resize + cvtColor menggunakan UMat agar berjalan di iGPU via OpenCL.
        """
        h, w = frame.shape[:2]
        ...
        return small_rgb.get(), scale

    def detect(self, frame):
        """
        Proses satu frame.
        Return: list of person-dict (0 atau 1 orang).
        """
        h, w = frame.shape[:2]
        ...
        # PoseLandmarker returns list of list (multi-pose), ambil yang pertama
...
# ============================================================
# ARUCO DETECTOR  (Hysteresis anti-flicker + EMA smoothing)
# ============================================================
class ArUcoDetector:
    """
    ArUco detector dengan stabilisasi hysteresis.

    Anti-flicker strategy (nol overhead — hanya counter logic):
      - confirm_frames: marker harus terdeteksi N frame berturut-turut
        sebelum dianggap "ada" → mencegah false-positive 1-frame.
      - lose_frames: marker harus hilang N frame berturut-turut
        sebelum dianggap "pergi" → mencegah dropout sesaat.
      - EMA smoothing pada posisi center → label tidak jittery.
    """

    def __init__(self, dict_type=ARUCO_DICT_TYPE,
                 confirm_frames=3, lose_frames=45, ema_alpha=0.18):
        dict_key = ARUCO_DICT_MAP.get(dict_type, cv2.aruco.DICT_4X4_50)
        self.aruco_dict = cv2.aruco.getPredefinedDictionary(dict_key)

        # Tune parameters untuk wide-angle camera (toleran distorsi tapi ketat false-positive)
        params = cv2.aruco.DetectorParameters()
        params.adaptiveThreshWinSizeMin  = 3
        params.adaptiveThreshWinSizeMax  = 23
        params.adaptiveThreshWinSizeStep = 4
        params.minMarkerPerimeterRate    = 0.02    # tidak terlalu kecil agar kurang false-positive
        params.maxMarkerPerimeterRate    = 4.0
        params.polygonalApproxAccuracyRate = 0.05  # lebih ketat bentuk persegi
        params.cornerRefinementMethod   = cv2.aruco.CORNER_REFINE_SUBPIX
        params.errorCorrectionRate       = 0.6     # kurangi agar tidak over-correct noise
        params.perspectiveRemovePixelPerCell = 6
        params.minDistanceToBorder       = 3       # marker harus cukup jauh dari pinggir
        ...
        self.ema_alpha      = ema_alpha   # 0→sangat halus, 1→tanpa smoothing

        # Per-marker state: {mid: {seen, lost, confirmed, data, smooth_center}}
        self._tracker = {}

    def detect(self, frame):
        """Detect ArUco markers with hysteresis anti-flicker."""
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        corners, ids, _ = self.detector.detectMarkers(gray)

        # Kumpulkan raw deteksi frame ini
        current = {}
        if ids is not None:
            for i, marker_id in enumerate(ids.flatten()):
                mid = int(marker_id)
                # Filter: hanya terima ID yang valid
                if mid not in ARUCO_VALID_IDS:
                    continue
                pts = corners[i][0]
                cx = int(np.mean(pts[:, 0]))
                cy = int(np.mean(pts[:, 1]))
                name = ID_MAP.get(mid, DEFAULT_NAME)
                current[mid] = {
                    'id': mid, 'name': name,
                    'corners': pts, 'center': (cx, cy),
                }

        # ── Update tracker per marker ──
        all_ids = set(self._tracker.keys()) | set(current.keys())

        for mid in all_ids:
            if mid not in self._tracker:
                self._tracker[mid] = {
                    "seen": 0, "lost": 0,
                    "confirmed": False,
                    "data": None, "smooth_center": None,
                }
            t = self._tracker[mid]

            if mid in current:
                # Terdeteksi frame ini
                t["seen"] = min(t["seen"] + 1, self.confirm_frames + 5)
                t["lost"] = 0

                # EMA smooth center
                raw_cx, raw_cy = current[mid]['center']
                if t["smooth_center"] is not None:
                    sx, sy = t["smooth_center"]
                    a = self.ema_alpha
                    scx = int(a * raw_cx + (1 - a) * sx)
                    scy = int(a * raw_cy + (1 - a) * sy)
                else:
                    scx, scy = raw_cx, raw_cy
                t["smooth_center"] = (scx, scy)

                # Simpan data dengan smoothed center
                data = current[mid].copy()
                data['center'] = (scx, scy)
                t["data"] = data

                # Konfirmasi setelah cukup frame berturut-turut
                if t["seen"] >= self.confirm_frames:
                    t["confirmed"] = True
            else:
                # Tidak terdeteksi frame ini
                t["seen"] = max(t["seen"] - 1, 0)
                t["lost"] += 1

                # Hilangkan setelah cukup lama tidak terdeteksi
                if t["lost"] >= self.lose_frames:
                    t["confirmed"] = False

        # Bersihkan marker yang sudah lama sekali hilang
        stale = [mid for mid, t in self._tracker.items()
                 if t["lost"] > self.lose_frames * 3 and not t["confirmed"]]
        for mid in stale:
            del self._tracker[mid]

        # Return hanya yang confirmed
        return [t["data"] for mid, t in self._tracker.items()
                if t["confirmed"] and t["data"] is not None]
...
# ============================================================
# ROI DATA CLASS  (Polygon 4 titik — trapesium/jajar genjang/kotak)
# ============================================================
class ROIZone:
    """
    ROI berbentuk polygon 4 titik (quadrilateral).
    Bisa berbentuk kotak, trapesium, jajar genjang, dll.
    points: list of 4 tuples [(x1,y1), (x2,y2), (x3,y3), (x4,y4)]
    Urutan: klik searah jarum jam atau berlawanan.
    """
    def __init__(self, points: list, name: str = "",
        ...
        self.name = name
        self.color = color
        # Cache numpy contour untuk cv2.pointPolygonTest
        self._contour = np.array(points, dtype=np.int32)

    def contains_point(self, px: int, py: int) -> bool:
        """Cek apakah titik (px, py) ada di dalam polygon."""
        return cv2.pointPolygonTest(self._contour, (float(px), float(py)), False) >= 0
    ...
    def contains_bbox(self, bx1: int, by1: int, bx2: int, by2: int,
        ...
        if mode == "center":
            ...
            # Cek apakah salah satu corner bbox ada di dalam polygon
            for px, py in [(bx1, by1), (bx2, by1), (bx1, by2), (bx2, by2)]:
                if self.contains_point(px, py):
                    return True
            # Cek apakah center bbox ada di dalam polygon
            return self.contains_point((bx1+bx2)//2, (by1+by2)//2)
        elif mode == "full":
            # Semua corner bbox harus di dalam polygon
            for px, py in [(bx1, by1), (bx2, by1), (bx1, by2), (bx2, by2)]:
    ...
    def bounding_rect(self):
        """Return axis-aligned bounding rectangle (x1, y1, x2, y2)."""
        xs = [p[0] for p in self.points]
        ys = [p[1] for p in self.points]
        return min(xs), min(ys), max(xs), max(ys)

    def to_dict(self) -> dict:
        return {"points": list(self.points), "name": self.name}

    @classmethod
    def from_dict(cls, d: dict) -> "ROIZone":
        # Backward compatible: support lama (x1,y1,x2,y2) → konversi ke polygon
        if "points" in d:
            pts = [tuple(p) for p in d["points"]]
        else:
            x1, y1, x2, y2 = d["x1"], d["y1"], d["x2"], d["y2"]
            pts = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
        return cls(points=pts, name=d.get("name", ""))
...
class YOLODetector:
    ...
    def _load_yolo_model(self, model_path):
        ...
        # Prioritas 1: CUDA (NVIDIA RTX 2050)
        try:
            import torch
            if torch.cuda.is_available():
                gpu_name = torch.cuda.get_device_name(0)
                self.device_used = f"CUDA ({gpu_name})"
                print(f"[INFO] Loading YOLO model via CUDA: {model_path}")
                print(f"[INFO]   GPU: {gpu_name}")
                model = YOLO(model_path)
                model.to('cuda')
                return model
        except Exception as e:
            print(f"[WARN] CUDA gagal: {e}")
        ...
        try:
            ...
            if 'DmlExecutionProvider' in providers:
                ...
                if not os.path.exists(onnx_path):
                    print(f"[INFO] Exporting {model_name} ke ONNX...")
                ...
                print(f"[INFO] Loading YOLO model via DirectML: {onnx_path}")
                return YOLO(onnx_path, task='detect')
        except ImportError:
            pass

        # Fallback: CPU
        print(f"[INFO] Loading YOLO model via CPU: {model_path}")
...
# ============================================================
# ROI MANAGER  (Polygon 4-click mode)
# ============================================================
class ROIManager:
    def __init__(self):
        self.zones: List[ROIZone] = []
        # State untuk mode 4-click polygon
        self._click_points = []    # titik yang sudah diklik (max 4)
        self._mouse_pos    = None  # posisi mouse saat ini (untuk preview)
    ...
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            print(f"[ROI] Titik {n}/4: ({x},{y})")

            if n == 4:
                # 4 titik sudah lengkap → buat ROI polygon
                idx   = len(self.zones)
                color = ROI_PALETTE[idx % len(ROI_PALETTE)]
                name  = f"ROI-{idx + 1}"
                pts   = list(self._click_points)
                self.zones.append(ROIZone(points=pts, name=name, color=color))
                print(f"[ROI] Ditambah: {name} (polygon 4 titik)")
                self._click_points = []
        elif event == cv2.EVENT_RBUTTONDOWN:
            # Klik kanan = batalkan titik terakhir
            if self._click_points:
                removed = self._click_points.pop()
                print(f"[ROI] Undo titik: {removed} ({len(self._click_points)}/4)")

    def draw_in_progress(self, frame):
        """Draw preview polygon saat user sedang mengklik titik-titik."""
        ...
        for i, (px, py) in enumerate(pts):
            cv2.circle(frame, (px, py), 6, ROI_ACTIVE_COLOR, -1, cv2.LINE_AA)
            cv2.putText(frame, str(i+1), (px + 8, py - 8),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, ROI_ACTIVE_COLOR, 1, cv2.LINE_AA)

        # Gambar garis antar titik
        for i in range(n - 1):
            cv2.line(frame, pts[i], pts[i+1], ROI_ACTIVE_COLOR, 2, cv2.LINE_AA)

        # Preview: garis dari titik terakhir ke mouse
        if self._mouse_pos and n < 4:
            cv2.line(frame, pts[-1], self._mouse_pos, (150, 255, 150), 1, cv2.LINE_AA)
            # Jika sudah 3 titik, preview garis penutup juga
            if n == 3:
                cv2.line(frame, self._mouse_pos, pts[0], (150, 255, 150), 1, cv2.LINE_AA)

        # Hint text
        hint = f"Klik titik {n+1}/4 (klik kanan = undo)"
        if n == 3:
            hint = "Klik titik 4/4 untuk selesaikan polygon"
        cv2.putText(frame, hint, (10, frame.shape[0] - 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, ROI_ACTIVE_COLOR, 1, cv2.LINE_AA)

    def draw_zones(self, frame):
        for i, roi in enumerate(self.zones):
            color = roi.color
            pts_np = np.array(roi.points, dtype=np.int32)
            ...
            # Corner marks (L-shape di setiap titik)
            for j in range(4):
                cx, cy = roi.points[j]
                # Arah ke titik sebelumnya dan sesudahnya
                prev_pt = roi.points[(j - 1) % 4]
                next_pt = roi.points[(j + 1) % 4]
                cl = 15  # panjang corner mark
                # Vektor ke prev & next, dinormalisasi
                for target in [prev_pt, next_pt]:
                    dx = target[0] - cx
                    dy = target[1] - cy
                    length = max(1, (dx*dx + dy*dy) ** 0.5)
                    ex = int(cx + dx / length * cl)
                    ey = int(cy + dy / length * cl)
                    cv2.line(frame, (cx, cy), (ex, ey), color, 3, cv2.LINE_AA)

            # Label
            label = roi.name if roi.name else f"Zone {i+1}"
            font  = cv2.FONT_HERSHEY_SIMPLEX
            # Posisi label: titik paling kiri-atas
            min_x = min(p[0] for p in roi.points)
            min_y = min(p[1] for p in roi.points)
            (tw, th), _ = cv2.getTextSize(label, font, 0.55, 2)
            lx, ly = min_x + 6, min_y + th + 6
    ...
    def delete_last(self):
        if self._click_points:
            self._click_points.clear()
            print("[ROI] Titik polygon dibatalkan.")
        elif self.zones:
...
def draw_mediapipe_skeleton(frame, person: dict, style: str = "full"):
    ...
    Menggunakan Tasks API baru (list[NormalizedLandmark] + Connection objects).
    style:
    ...
    if pose_lm and style in ("full", "minimal"):
        if style == "full":
            mp_drawing.draw_landmarks(
                frame, pose_lm, POSE_CONNECTIONS,
                DrawingSpec(color=SKELETON_POSE_LM, thickness=2, circle_radius=4),
                DrawingSpec(color=SKELETON_POSE_CN, thickness=2, circle_radius=2)
            )
        else:
            # Minimal: hanya gambar koneksi tubuh (skip kepala 0-10)
            BODY_CONNECTIONS = [
                conn for conn in POSE_CONNECTIONS
                if conn.start >= 11 and conn.end >= 11
            ]
            mp_drawing.draw_landmarks(
                frame, pose_lm, BODY_CONNECTIONS,
                DrawingSpec(color=SKELETON_POSE_LM, thickness=2, circle_radius=4),
                DrawingSpec(color=SKELETON_POSE_CN, thickness=2, circle_radius=2)
            )

    # --- Gambar Tangan Kanan ---
    if r_hand and style in ("full", "hands"):
        mp_drawing.draw_landmarks(
            frame, r_hand, HAND_CONNECTIONS,
            DrawingSpec(color=SKELETON_RHAND_LM, thickness=2, circle_radius=4),
            DrawingSpec(color=SKELETON_RHAND_CN, thickness=2, circle_radius=2)
        )

    # --- Gambar Tangan Kiri ---
    if l_hand and style in ("full", "hands"):
        mp_drawing.draw_landmarks(
            frame, l_hand, HAND_CONNECTIONS,
            DrawingSpec(color=SKELETON_LHAND_LM, thickness=2, circle_radius=4),
            DrawingSpec(color=SKELETON_LHAND_CN, thickness=2, circle_radius=2)
        )
...
def draw_mediapipe_skeleton_dim(frame, person: dict, style: str = "full"):
    ...
    dim_lm  = DrawingSpec(color=(60, 50, 40), thickness=1, circle_radius=2)
    dim_cn  = DrawingSpec(color=(60, 30, 50), thickness=1, circle_radius=1)
    dim_h   = DrawingSpec(color=(40, 15, 5),  thickness=1, circle_radius=2)
    dim_hc  = DrawingSpec(color=(40, 30, 80), thickness=1, circle_radius=1)

    if pose_lm and style in ("full", "minimal"):
        mp_drawing.draw_landmarks(frame, pose_lm, POSE_CONNECTIONS, dim_lm, dim_cn)
    if r_hand and style in ("full", "hands"):
        mp_drawing.draw_landmarks(frame, r_hand, HAND_CONNECTIONS, dim_h, dim_hc)
    if l_hand and style in ("full", "hands"):
        mp_drawing.draw_landmarks(frame, l_hand, HAND_CONNECTIONS, dim_h, dim_hc)
...
def main():
    ...
    # Auto-select: IP Camera Lite (MJPEG HTTP) vs CameraStream (webcam/file)
    _is_ipcam = (isinstance(CAMERA_INDEX, str)
                 and CAMERA_INDEX.startswith("http")
                 and "/video" in CAMERA_INDEX
                 and ":808" in CAMERA_INDEX)
    if _is_ipcam:
        print("[INFO] Terdeteksi IP Camera Lite — menggunakan IPCameraStream")
        cap = IPCameraStream(
            url=CAMERA_INDEX,
            username=IP_CAM_USERNAME,
            password=IP_CAM_PASSWORD,
            width=FRAME_WIDTH,
            height=FRAME_HEIGHT,
        )
    else:
    ...
    # Tunggu frame pertama dari IP Camera (maks 5 detik)
    if _is_ipcam:
        print("[IPCam] Menunggu frame pertama...")
        _wait_start = time.time()
        while not cap.ret and not cap.stopped and (time.time() - _wait_start) < 5:
            time.sleep(0.1)
        if not cap.ret:
            print("[ERROR] Tidak ada frame dari IP Camera dalam 5 detik!")
            sys.exit(1)
        print(f"[IPCam] Frame pertama diterima! Resolusi: {cap.actual_w}x{cap.actual_h}")
    ...
    operator_tracker = OperatorTracker(max_lost_frames=45, distance_threshold=250)
    ...
    try:
        while not cap.stopped:
            ...
            # ========================================
            # STEP 0a: Resize frame (IP cam 1920x1080 → target)
            # ========================================
            h_raw, w_raw = frame.shape[:2]
            if w_raw > FRAME_WIDTH or h_raw > FRAME_HEIGHT:
